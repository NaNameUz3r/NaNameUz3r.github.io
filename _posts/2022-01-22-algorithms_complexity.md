...
- Потянуться
- Надеть носки
- Поехать на работу (или дойти до десктопа и начать работать :) )
- Прерваться на обед
...
Только в асимптотическом анализе сложности *компьютерных алгоритмов* за эту точку принимается бесконечность, потому что нас интересует не фактическое время выполнения конкретного алгоритма (секунды,etc), а то как будет меняться эффективность работы, если мы будем постоянно и бесконечно повышать размер входных данных.
...
Пока мы рассмотрим фундаментальные меры сложности, используемые повсеместно, которые выражаются через O-большое.
